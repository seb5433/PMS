...
def load_tasks(project):
	"""Carga las tareas a partir de un objecto proyecto 
	* task_id
	* name
	* duration
	* predecessor ids (with each predecessor ID separated by a comma)
	...
	for actividad in project.actividades:
		task = Task (actividad.id, actividad.nombre, parsear_predecesores(actividad.id))
...
def parsear_predecesores (predecessor_ids_string):

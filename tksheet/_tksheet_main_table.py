...
class MainTable(tk.Canvas):
    def __init__(self,
        ...
        if not self.data_ref:
            if isinstance(total_rows, int) and isinstance(total_cols, int) and total_rows > 0 and total_cols > 0:
                self.data_ref = [list(repeat("", total_cols))
    ...
    def create_ctrl_outline(self, x1, y1, x2, y2, fill, dash, width, outline, tag):
        if self.hidd_ctrl_outline:
            ...
            if sh:
                self.itemconfig(t, fill=fill, dash=dash,
                                width=width, outline=outline, tag=tag)
            else:
                self.itemconfig(t, fill=fill, dash=dash, width=width,
                                outline=outline, tag=tag, state="normal")
            self.lift(t)
        else:
            t = self.create_rectangle(
                x1, y1, x2, y2, fill=fill, dash=dash, width=width, outline=outline, tag=tag)
        ...
        if isinstance(currently_selected[0], int) or currently_selected[0] == "column":
            for item in chain(self.find_withtag("CellSelectFill"), self.find_withtag("Current_Outside"), self.find_withtag("ColSelectFill")):
                ...
                if alltags[0] == "CellSelectFill" or alltags[0] == "Current_Outside":
                    boxes[tuple(int(e)
                                for e in alltags[1].split("_") if e)] = "cells"
                elif alltags[0] == "ColSelectFill":
                    boxes[tuple(int(e)
                                for e in alltags[1].split("_") if e)] = "cols"
            maxrows = 0
            ...
            for item in self.find_withtag("RowSelectFill"):
                boxes[tuple(int(e) for e in self.gettags(
                    item)[1].split("_") if e)] = "rows"
            return boxes
    ...
    def ctrl_c(self, event=None):
        ...
        if currently_selected:
            ...
            writer = csv_module.writer(
                s, dialect=csv_module.excel_tab, lineterminator="\n")
            rows = []
            if isinstance(currently_selected[0], int) or currently_selected[0] == "column":
                ...
                if self.extra_begin_ctrl_c_func is not None:
                    try:
                        self.extra_begin_ctrl_c_func(
                            ("begin_ctrl_c", boxes, currently_selected))
                    except:
                        return
            ...
            if self.extra_end_ctrl_c_func is not None:
                self.extra_end_ctrl_c_func(
                    ("end_ctrl_c", boxes, currently_selected, rows))
        ...
        if self.anything_selected():
                ...
                if self.extra_begin_ctrl_x_func is not None:
                    try:
                        self.extra_begin_ctrl_x_func(
                            ("begin_ctrl_x", boxes, currently_selected))
                    except:
                        return
                if self.all_columns_displayed:
                    for rn in range(maxrows):
                        ...
                        for r1, c1, r2, c2 in boxes:
                            ...
                            for c in range(c1, c2):
                                try:
                                    ...
                                    if self.undo_enabled:
                                        undo_storage[(
                                            data_ref_rn, self.displayed_columns[c])] = sx
                                except:
                                    row.append("")
                        writer.writerow(row)
                        rows.append(row)
                        ...
                        for rn in range(r2 - r1):
                            ...
                            for c in range(c1, c2):
                                ...
                                try:
                                    ...
                                    continue
            if self.undo_enabled:
                self.undo_storage.append(zlib.compress(pickle.dumps(
                    ("edit_cells", undo_storage, tuple(boxes.items()), currently_selected))))
            self.clipboard_clear()
            self.clipboard_append(s.getvalue())
            self.update()
            self.refresh()
            ...
            if self.extra_end_ctrl_x_func is not None:
                self.extra_end_ctrl_x_func(
                    ("end_ctrl_x", boxes, currently_selected, rows))
    ...
    def ctrl_v(self, event=None):
        ...
        try:
            ...
            return
        data = list(csv_module.reader(io.StringIO(data),
        ...
        if self.expand_sheet_if_paste_too_big:
            ...
            if x1 + numcols > len(self.col_positions) - 1:
                ...
                if isinstance(self.paste_insert_column_limit, int) and self.paste_insert_column_limit < len(self.col_positions) - 1 + added_cols:
                    added_cols = self.paste_insert_column_limit - \
            ...
            if y1 + numrows > len(self.row_positions) - 1:
                ...
                if isinstance(self.paste_insert_row_limit, int) and self.paste_insert_row_limit < len(self.row_positions) - 1 + added_rows:
                    added_rows = self.paste_insert_row_limit - \
        ...
        if self.extra_begin_ctrl_v_func is not None or self.extra_end_ctrl_v_func is not None:
            rows = [[data[ndr][ndc] for ndc, c in enumerate(
                range(x1, x1 + numcols))] for ndr, r in enumerate(range(y1, y1 + numrows))]
        if self.extra_begin_ctrl_v_func is not None:
            try:
                self.extra_begin_ctrl_v_func(
                    ("begin_ctrl_v", currently_selected, rows))
            except:
                return
        if self.all_columns_displayed:
            for ndr, r in enumerate(range(y1, y1 + numrows)):
                for ndc, c in enumerate(range(x1, x1 + numcols)):
                    if r > len(self.data_ref) - 1:
                        self.data_ref.extend(
                            [list(repeat("", c + 1)) for r in range((r + 1) - len(self.data_ref))])
                    elif c > len(self.data_ref[r]) - 1:
                        self.data_ref[r].extend(
                            list(repeat("", (c + 1) - len(self.data_ref[r]))))
                    if (
            ...
            for ndr, r in enumerate(range(y1, y1 + numrows)):
                for ndc, c in enumerate(range(x1, x1 + numcols)):
                    ...
                    if (
                        ...
                        continue
                    if self.undo_enabled:
                        undo_storage[(r, self.displayed_columns[c])
                                     ] = f"{self.data_ref[r][self.displayed_columns[c]]}"
        ...
        self.create_current(
            y1, x1, type_="cell", inside=True if numrows > 1 or numcols > 1 else False)
        self.see(r=y1, c=x1, keep_yscroll=False, keep_xscroll=False,
                 bottom_right_corner=False, check_cell_visibility=True, redraw=False)
        self.refresh()
        if self.extra_end_ctrl_v_func is not None:
            self.extra_end_ctrl_v_func(
                ("end_ctrl_v", currently_selected, rows))
        ...
        if self.anything_selected():
                ...
                if self.extra_begin_delete_key_func is not None:
                    try:
                        self.extra_begin_delete_key_func(
                            ("begin_delete_key", boxes, currently_selected))
                    except:
                        return
            ...
            if self.extra_end_delete_key_func is not None:
                self.extra_end_delete_key_func(
                    ("end_delete_key", boxes, currently_selected))

    def ctrl_z(self, event=None):
        if self.undo_storage:
            if not isinstance(self.undo_storage[-1], (tuple, dict)):
                undo_storage = pickle.loads(
                    zlib.decompress(self.undo_storage[-1]))
            else:
            ...
            if self.extra_begin_ctrl_z_func is not None:
                try:
                    self.extra_begin_ctrl_z_func(
                        ("begin_ctrl_z", undo_storage[0], undo_storage))
                except:
                    return
            ...
            if undo_storage[0] in ("edit_cells", "edit_cells_paste"):
                for (r, c), v in undo_storage[1].items():
                    ...
                    if (r, c) in self.cell_options and 'dropdown' in self.cell_options[(r, c)]:
                        self.cell_options[(
                            r, c)]['dropdown'][0].set_displayed(v)
                start_row = float("inf")
                start_col = float("inf")
                ...
                if undo_storage[0] == "edit_cells_paste" and self.expand_sheet_if_paste_too_big:
                    if undo_storage[4][0] > 0:
                        self.del_row_positions(
                            len(self.row_positions) - 1 - undo_storage[4][0], undo_storage[4][0])
                    ...
                    if undo_storage[4][1] > 0:
                        ...
                        for rn in range(len(self.data_ref)):
                            self.data_ref[rn][:] = self.data_ref[rn][:-
                                                                     quick_added_cols]
                        if not self.all_columns_displayed:
                ...
                if undo_storage[3]:
                    if isinstance(undo_storage[3][0], int):
                        self.create_current(undo_storage[3][0], undo_storage[3][1], type_="cell", inside=True if self.cell_selected(
                            undo_storage[3][0], undo_storage[3][1]) else False)
                    elif undo_storage[3][0] == "column":
                        self.create_current(
                            0, undo_storage[3][1], type_="col", inside=True)
                    elif undo_storage[3][0] == "row":
                        self.create_current(
                            undo_storage[3][1], 0, type_="row", inside=True)
                else:
                    self.create_current(start_row, start_col, type_="cell", inside=True if self.cell_selected(
                        start_row, start_col) else False)
                self.see(r=start_row, c=start_col, keep_yscroll=False, keep_xscroll=False,
                         bottom_right_corner=False, check_cell_visibility=True, redraw=False)

            elif undo_storage[0] == "move_rows":
                rhs = [int(b - a) for a, b in zip(self.row_positions,
                ...
                if rm1start > ins_row:
                    ...
                    self.row_positions = list(accumulate(
                        chain([0], (height for height in rhs))))
                    self.create_current(ins_row, 0, type_="row", inside=True)
                    self.create_selected(
                        ins_row, 0, ins_row + totalrows, len(self.col_positions) - 1, "rows")
                else:
                    rhs[ins_row:ins_row] = rhs[rm1start:rm1end]
                    rhs[rm1start:rm1end] = []
                    self.row_positions = list(accumulate(
                        chain([0], (height for height in rhs))))
                    self.create_current(ins_row - totalrows,
                                        0, type_="row", inside=True)
                    self.create_selected(
                        ins_row - totalrows, 0, ins_row, len(self.col_positions) - 1, "rows")
                self.see(r=orig_ins_row, c=0, keep_yscroll=False, keep_xscroll=True,
                         bottom_right_corner=False, check_cell_visibility=True, redraw=False)
                rowsiter = tuple(range(rm1start, rm1end))
                rowset = set(rowsiter)
                popped_ri = {t1: t2 for t1,
                             t2 in self.RI.cell_options.items() if t1 in rowset}
                popped_cell = {
                    t1: t2 for t1, t2 in self.cell_options.items() if t1[0] in rowset}
                popped_row = {t1: t2 for t1,
                              t2 in self.row_options.items() if t1 in rowset}

                popped_ri = {t1: self.RI.cell_options.pop(
                    t1) for t1 in popped_ri}
                popped_cell = {t1: self.cell_options.pop(
                    t1) for t1 in popped_cell}
                popped_row = {t1: self.row_options.pop(
                    t1) for t1 in popped_row}

                self.RI.cell_options = {
                    t1 if t1 < rm1start else t1 - totalrows: t2 for t1, t2 in self.RI.cell_options.items()}
                self.RI.cell_options = {
                    t1 if t1 < ins_row else t1 + totalrows: t2 for t1, t2 in self.RI.cell_options.items()}

                self.row_options = {t1 if t1 < rm1start else t1 -
                                    totalrows: t2 for t1, t2 in self.row_options.items()}
                self.row_options = {t1 if t1 < ins_row else t1 +
                                    totalrows: t2 for t1, t2 in self.row_options.items()}
                ...
                cws = [int(b - a) for a, b in zip(self.col_positions,
                ...
                if rm1start > ins_col:
                    ...
                    cws[rm2start:rm2end] = []
                    self.col_positions = list(accumulate(
                        chain([0], (width for width in cws))))
                    self.create_current(0, ins_col, type_="col", inside=True)
                    self.create_selected(0, ins_col, len(
                        self.row_positions) - 1, ins_col + totalcols, "cols")
                else:
                    cws[ins_col:ins_col] = cws[rm1start:rm1end]
                    cws[rm1start:rm1end] = []
                    self.col_positions = list(accumulate(
                        chain([0], (width for width in cws))))
                    self.create_current(
                        0, ins_col - totalcols, type_="col", inside=True)
                    self.create_selected(
                        0, ins_col - totalcols, len(self.row_positions) - 1, ins_col, "cols")
                self.see(r=0, c=orig_ins_col, keep_yscroll=True, keep_xscroll=False,
                         bottom_right_corner=False, check_cell_visibility=True, redraw=False)
                colsiter = tuple(range(rm1start, rm1end))
                colset = set(colsiter)
                popped_ch = {t1: t2 for t1,
                             t2 in self.CH.cell_options.items() if t1 in colset}
                popped_cell = {
                    t1: t2 for t1, t2 in self.cell_options.items() if t1[1] in colset}
                popped_col = {t1: t2 for t1,
                              t2 in self.col_options.items() if t1 in colset}

                popped_ch = {t1: self.CH.cell_options.pop(
                    t1) for t1 in popped_ch}
                popped_cell = {t1: self.cell_options.pop(
                    t1) for t1 in popped_cell}
                popped_col = {t1: self.col_options.pop(
                    t1) for t1 in popped_col}

                self.CH.cell_options = {
                    t1 if t1 < rm1start else t1 - totalcols: t2 for t1, t2 in self.CH.cell_options.items()}
                self.CH.cell_options = {
                    t1 if t1 < ins_col else t1 + totalcols: t2 for t1, t2 in self.CH.cell_options.items()}

                self.col_options = {t1 if t1 < rm1start else t1 -
                                    totalcols: t2 for t1, t2 in self.col_options.items()}
                self.col_options = {t1 if t1 < ins_col else t1 +
                                    totalcols: t2 for t1, t2 in self.col_options.items()}
                ...
                self.data_ref[undo_storage[1]['data_row_num']:undo_storage[1]
                              ['data_row_num'] + undo_storage[1]['numrows']] = []
                try:
                    self.my_row_index[undo_storage[1]['data_row_num']:undo_storage[1]
                                      ['data_row_num'] + undo_storage[1]['numrows']] = []
                except:
                    pass
                ...
                idx = undo_storage[1]['sheet_row_num'] + \
                    undo_storage[1]['numrows']
                self.cell_options = {(rn if rn < idx else rn - numrows, cn)                                     : t2 for (rn, cn), t2 in self.cell_options.items()}
                self.row_options = {rn if rn < idx else rn -
                                    numrows: t for rn, t in self.row_options.items()}
                self.RI.cell_options = {
                    rn if rn < idx else rn - numrows: t for rn, t in self.RI.cell_options.items()}
                self.refresh_dropdowns()
                if len(self.row_positions) > 1:
                    start_row = undo_storage[1]['sheet_row_num'] if undo_storage[1]['sheet_row_num'] < len(
                        self.row_positions) - 1 else undo_storage[1]['sheet_row_num'] - 1
                    self.RI.select_row(start_row)
                    self.see(r=start_row, c=0, keep_yscroll=False, keep_xscroll=False,
                             bottom_right_corner=False, check_cell_visibility=True, redraw=False)
                ...
                idx = undo_storage[1]['sheet_col_num'] + \
                    undo_storage[1]['numcols']
                self.cell_options = {(rn, cn if cn < idx else cn - numcols)                                     : t2 for (rn, cn), t2 in self.cell_options.items()}
                self.col_options = {cn if cn < idx else cn -
                                    numcols: t for cn, t in self.col_options.items()}
                self.CH.cell_options = {
                    cn if cn < idx else cn - numcols: t for cn, t in self.CH.cell_options.items()}
                self.refresh_dropdowns()
                if len(self.col_positions) > 1:
                    start_col = undo_storage[1]['sheet_col_num'] if undo_storage[1]['sheet_col_num'] < len(
                        self.col_positions) - 1 else undo_storage[1]['sheet_col_num'] - 1
                    self.CH.select_col(start_col)
                    self.see(r=0, c=start_col, keep_yscroll=False, keep_xscroll=False,
                             bottom_right_corner=False, check_cell_visibility=True, redraw=False)
                ...
                self.reselect_from_get_boxes(
                    undo_storage[1]['selection_boxes'])
                self.refresh_dropdowns()
            ...
            if self.extra_end_ctrl_z_func is not None:
                self.extra_end_ctrl_z_func(
                    ("end_ctrl_z", undo_storage[0], undo_storage))
    ...
    def see(self,
        ...
        if check_cell_visibility:
            yvis, xvis = self.cell_is_completely_visible(
                r=r, c=c, separate_axes=True)
        else:
        ...
        if not yvis:
            if bottom_right_corner:
                ...
                if r is not None and not keep_yscroll:
                    args = (
                        "moveto", self.row_positions[r] / (self.row_positions[-1] + self.empty_vertical))
                    if args[1] > 1:
                        args[1] = args[1] - 1
                    self.yview(*args)
                    self.RI.yview(*args)
        ...
        if not xvis:
            if bottom_right_corner:
                ...
                if c is not None and not keep_xscroll:
                    args = (
                        "moveto", self.col_positions[c] / (self.col_positions[-1] + self.empty_horizontal))
                    self.xview(*args)
                    self.CH.xview(*args)
                    if redraw:
                        need_redraw = True
    ...
    def move_down(self):
        ...
        if currently_selected:
            ...
            if (
                ...
                self.see(r + 1, c, keep_xscroll=True,
                         bottom_right_corner=True, check_cell_visibility=True)
    ...
    def add_selection(self, r, c, redraw=False, run_binding_func=True, set_as_current=False):
        ...
        if set_as_current:
            ...
            if items:
                ...
                if alltags[2] == "cell":
                    r1, c1, r2, c2 = tuple(int(e)
                                           for e in alltags[1].split("_") if e)
                    add_sel = (r1, c1)
                else:
                    add_sel = tuple()
            else:
                add_sel = tuple()
            self.create_current(
                r, c, type_="cell", inside=True if self.cell_selected(r, c) else False)
            if add_sel:
                self.add_selection(
                    add_sel[0], add_sel[1], redraw=False, run_binding_func=False, set_as_current=False)
        else:
    ...
    def toggle_select_cell(self, row, column, add_selection=True, redraw=True, run_binding_func=True, set_as_current=True):
        if add_selection:
            if self.cell_selected(row, column, inc_rows=True, inc_cols=True):
                ...
                self.add_selection(r=row, c=column, redraw=redraw,
                                   run_binding_func=run_binding_func, set_as_current=set_as_current)
        else:
            if self.cell_selected(row, column, inc_rows=True, inc_cols=True):
                self.deselect(r=row, c=column, redraw=redraw)
            else:
    ...
    def align_rows(self, rows=[], align="global", align_index=False):
    ...
    # "center", "w", "e" or "global"
    def align_columns(self, columns=[], align="global", align_header=False):
    ...
    # "center", "w", "e" or "global"
    def align_cells(self, row=0, column=0, cells=[], align="global"):
        if align == "global":
            if cells:
                for r, c in cells:
    ...
    def highlight_cols(self, cols=[], bg=None, fg=None, highlight_header=False, redraw=False):
        ...
        if redraw:
            self.main_table_redraw_grid_and_text(
                redraw_header=highlight_header)
    ...
    def highlight_rows(self, rows=[], bg=None, fg=None, highlight_index=False, redraw=False):
        ...
        if redraw:
            self.main_table_redraw_grid_and_text(
                redraw_row_index=highlight_index)

    def deselect(self, r=None, c=None, cell=None, redraw=True):
        deselected = tuple()
        deleted_boxes = {}
        if r == "all":
            deselected = ("deselect_all", tuple(
                self.delete_selection_rects().items()))
        elif r == "allrows":
            for item in self.find_withtag("RowSelectFill"):
                alltags = self.gettags(item)
                if alltags:
            ...
            if current and current[0] == "row":
                deleted_boxes[tuple(int(e) for e in self.get_tags_of_current()[
                                    1].split("_") if e)] = "cell"
                self.delete_current()
            ...
            current = self.find_withtag(
                "Current_Inside") + self.find_withtag("Current_Outside")
            ...
            for item in self.find_withtag("RowSelectFill"):
                ...
                if alltags:
                    ...
                    if not reset_current and current and curr_r1 >= r1 and curr_r1 < r2:
                        reset_current = True
                        deleted_boxes[curr_r1, curr_c1,
                                      curr_r2, curr_c2] = "cell"
                    deleted_boxes[r1, c1, r2, c2] = "rows"
            if reset_current:
                self.delete_current()
    ...
    def page_UP(self, event=None):
        ...
        if self.page_up_down_select_row:
            ...
            if self.RI.row_selection_enabled and (self.anything_selected(exclude_columns=True, exclude_cells=True) or not self.anything_selected()):
                ...
                self.see(r, box[1], keep_xscroll=True,
        ...
        if currently_selected[0] == "row":
            ...
            if r != 0 and self.RI.row_selection_enabled:
                if self.cell_is_completely_visible(r=r - 1, c=0):
                    ...
                    self.see(r - 1, 0, keep_xscroll=True,
                             check_cell_visibility=False)
        elif isinstance(currently_selected[0], int):
            ...
            if r == 0 and self.CH.col_selection_enabled:
                if not self.cell_is_completely_visible(r=r, c=0):
                    self.see(r, c, keep_xscroll=True,
                             check_cell_visibility=False)
        ...
        if currently_selected[0] == "row":
            ...
            if self.single_selection_enabled or self.toggle_selection_enabled:
                if self.cell_is_completely_visible(r=r, c=0):
                    ...
                    self.see(r, 0, keep_yscroll=True,
                             bottom_right_corner=True, check_cell_visibility=False)
            ...
            if c < len(self.col_positions) - 2 and self.CH.col_selection_enabled:
                if self.cell_is_completely_visible(r=0, c=c + 1):
                    ...
                    self.see(0, c + 1, keep_yscroll=True,
                             bottom_right_corner=False if self.arrow_key_down_right_scroll_page else True, check_cell_visibility=False)
        elif isinstance(currently_selected[0], int):
            r = currently_selected[0]
            c = currently_selected[1]
            if c < len(self.col_positions) - 2 and (self.single_selection_enabled or self.toggle_selection_enabled):
                if self.cell_is_completely_visible(r=r, c=c + 1):
                    ...
                    self.see(r, c + 1, keep_yscroll=True,
                             bottom_right_corner=False if self.arrow_key_down_right_scroll_page else True, check_cell_visibility=False)
        ...
        if currently_selected[0] == "row":
            ...
            if r < len(self.row_positions) - 2 and self.RI.row_selection_enabled:
                if self.cell_is_completely_visible(r=min(r + 2, len(self.row_positions) - 2), c=0):
                    ...
                    if r + 2 < len(self.row_positions) - 2 and (self.row_positions[r + 3] - self.row_positions[r + 2]) + (self.row_positions[r + 2] - self.row_positions[r + 1]) + 5 < self.winfo_height():
                        self.see(r + 2, 0, keep_xscroll=True,
                                 bottom_right_corner=True, check_cell_visibility=False)
            ...
            if self.single_selection_enabled or self.toggle_selection_enabled:
                if self.cell_is_completely_visible(r=0, c=c):
                    ...
                    self.see(0, c, keep_xscroll=True,
                             bottom_right_corner=True, check_cell_visibility=False)
        elif isinstance(currently_selected[0], int):
            r = currently_selected[0]
            c = currently_selected[1]
            if r < len(self.row_positions) - 2 and (self.single_selection_enabled or self.toggle_selection_enabled):
                if self.cell_is_completely_visible(r=min(r + 2, len(self.row_positions) - 2), c=c):
                    ...
                    self.select_cell(r + 1, c)
                    if r + 2 < len(self.row_positions) - 2 and (self.row_positions[r + 3] - self.row_positions[r + 2]) + (self.row_positions[r + 2] - self.row_positions[r + 1]) + 5 < self.winfo_height():
                        self.see(r + 2, c, keep_xscroll=True,
                                 bottom_right_corner=True, check_cell_visibility=False)
    ...
    def arrowkey_LEFT(self, event=None):
        ...
        if currently_selected[0] == "column":
            ...
            if c == 0 and self.RI.row_selection_enabled:
                if not self.cell_is_completely_visible(r=r, c=0):
                    self.see(r, c, keep_yscroll=True,
                             check_cell_visibility=False)
            ...
            if enable:
                for s2 in ("c", "C"):
                    for widget in (self, self.RI, self.CH):
                        widget.bind(
                            f"<{'Command' if USER_OS == 'Darwin' else 'Control'}-{s2}>", self.ctrl_c)
        ...
        if key is None or key == "cut":
            if enable:
                for s2 in ("x", "X"):
                    for widget in (self, self.RI, self.CH):
                        widget.bind(
                            f"<{'Command' if USER_OS == 'Darwin' else 'Control'}-{s2}>", self.ctrl_x)
        ...
        if key is None or key == "paste":
            if enable:
                for s2 in ("v", "V"):
                    for widget in (self, self.RI, self.CH):
                        widget.bind(
                            f"<{'Command' if USER_OS == 'Darwin' else 'Control'}-{s2}>", self.ctrl_v)
        ...
        if key is None or key == "undo":
            if enable:
                for s2 in ("z", "Z"):
                    for widget in (self, self.RI, self.CH):
                        widget.bind(
                            f"<{'Command' if USER_OS == 'Darwin' else 'Control'}-{s2}>", self.ctrl_z)
    ...
    def create_rc_menus(self):
        ...
        if self.rc_delete_row_enabled:
            self.RI.ri_rc_popup_menu.add_command(label="Borrar alumno",
                                                 font=self.popup_menu_font,
                                                 foreground=self.popup_menu_fg,
                                                 background=self.popup_menu_bg,
                                                 activebackground=self.popup_menu_highlight_bg,
        ...
        if self.rc_insert_row_enabled:
            self.RI.ri_rc_popup_menu.add_command(label="Insertar fila arriba",
                                                 font=self.popup_menu_font,
                                                 foreground=self.popup_menu_fg,
                                                 background=self.popup_menu_bg,
                                                 activebackground=self.popup_menu_highlight_bg,
            ...
            self.RI.ri_rc_popup_menu.add_command(label="Insertar fila debajo",
                                                 font=self.popup_menu_font,
                                                 foreground=self.popup_menu_fg,
                                                 background=self.popup_menu_bg,
                                                 activebackground=self.popup_menu_highlight_bg,
            ...
            self.empty_rc_popup_menu.add_command(label="Insertar Alumno",
                                                 font=self.popup_menu_font,
                                                 foreground=self.popup_menu_fg,
                                                 background=self.popup_menu_bg,
                                                 activebackground=self.popup_menu_highlight_bg,
    ...
    def mouse_motion(self, event):
        if (
            ...
            if self.RI.width_resizing_enabled and not mouse_over_resize:
                try:
                    x1, y1, x2, y2 = self.row_width_resize_bbox[0], self.row_width_resize_bbox[
                        1], self.row_width_resize_bbox[2], self.row_width_resize_bbox[3]
            ...
            if self.CH.height_resizing_enabled and not mouse_over_resize:
                try:
                    x1, y1, x2, y2 = self.header_height_resize_bbox[0], self.header_height_resize_bbox[
                        1], self.header_height_resize_bbox[2], self.header_height_resize_bbox[3]
                    if x >= x1 and y >= y1 and x <= x2 and y <= y2:
            ...
            if r < len(self.row_positions) - 1 and c < len(self.col_positions) - 1:
                if self.col_selected(c):
                    if self.rc_popup_menus_enabled:
                        self.CH.ch_rc_popup_menu.tk_popup(
                            event.x_root, event.y_root)
                elif self.row_selected(r):
                    if self.rc_popup_menus_enabled:
                        self.RI.ri_rc_popup_menu.tk_popup(
                            event.x_root, event.y_root)
        ...
        if self.hidd_resize_lines:
            ...
            if sh:
                ...
                self.itemconfig(t, width=width, fill=fill,
                                tag=tag, state="normal")
            self.lift(t)
        else:
            t = self.create_line(
                x1, y1, x2, y2, width=width, fill=fill, tag=tag)
            ...
            if rowsel < len(self.row_positions) - 1 and colsel < len(self.col_positions) - 1:
                ...
                if currently_selected and isinstance(currently_selected[0], int):
                    ...
                    if rowsel >= min_r and colsel >= min_c:
                        self.create_selected(
                            min_r, min_c, rowsel + 1, colsel + 1)
                    elif rowsel >= min_r and min_c >= colsel:
                        self.create_selected(
                            min_r, colsel, rowsel + 1, min_c + 1)
                    elif min_r >= rowsel and colsel >= min_c:
                        self.create_selected(
                            rowsel, min_c, min_r + 1, colsel + 1)
                    elif min_r >= rowsel and min_c >= colsel:
                        self.create_selected(
                            rowsel, colsel, min_r + 1, min_c + 1)
                else:
                ...
                if self.shift_selection_binding_func is not None:
                    self.shift_selection_binding_func(("shift_select_cells", ) + tuple(int(
                        e) for e in self.gettags(self.find_withtag("CellSelectFill"))[1].split("_") if e))
            ...
            if end_row < len(self.row_positions) - 1 and end_col < len(self.col_positions) - 1 and currently_selected and isinstance(currently_selected[0], int):
                ...
                if self.being_drawn_rect != rect:
                    ...
                    if self.drag_selection_binding_func is not None:
                        self.drag_selection_binding_func(("drag_select_cells", ) + tuple(int(
                            e) for e in self.gettags(self.find_withtag("CellSelectFill"))[1].split("_") if e))
            scrolled = False
            if self.data_ref:
                xcheck = self.xview()
    ...
    def GetTextWidth(self, txt):
        self.txt_measure_canvas.itemconfig(
            self.txt_measure_canvas_text, text=txt, font=self.my_font)
    ...
    def GetHdrTextWidth(self, txt):
        self.txt_measure_canvas.itemconfig(
            self.txt_measure_canvas_text, text=txt, font=self.my_hdr_font)
        b = self.txt_measure_canvas.bbox(self.txt_measure_canvas_text)
        return b[2] - b[0]
    ...
    def set_cell_size_to_text(self, r, c, only_set_if_too_small=False, redraw=True, run_binding=False):
        ...
        h = int(min_rh)
        w = int(min_cw)
        x = self.txt_measure_canvas.create_text(
            0, 0, text="", font=self.my_font)
        x2 = self.txt_measure_canvas.create_text(
            0, 0, text="", font=self.my_hdr_font)
        ...
        if txt:
            itmcon(x, text=txt)
            b = itmbbx(x)
            tw = b[2] - b[0] + 25 if (rn, cn) in self.cell_options and 'dropdown' in self.cell_options[(
                rn, cn)] else b[2] - b[0] + 5
            if b[3] - b[1] + 5 > h:
                h = b[3] - b[1] + 5
        else:
        ...
        if cell_needs_resize_w:
            ...
            self.col_positions[c + 2:] = [e + increment for e in islice(
                self.col_positions, c + 2, len(self.col_positions))]
            ...
            if run_binding and self.CH.column_width_resize_func is not None and old_width != new_width:
                self.CH.column_width_resize_func(
                    ("column_width_resize", c, old_width, new_width))
        if cell_needs_resize_h:
            ...
            self.row_positions[r + 2:] = [e + increment for e in islice(
                self.row_positions, r + 2, len(self.row_positions))]
            ...
            if run_binding and self.RI.row_height_resize_func is not None and old_height != new_height:
                self.RI.row_height_resize_func(
                    ("row_height_resize", r, old_height, new_height))
    ...
    def set_all_cell_sizes_to_text(self, include_index=False):
        ...
        self.txt_measure_canvas.delete(x2)
        self.row_positions = list(accumulate(
            chain([0], (height for height in rhs.values()))))
        self.col_positions = list(accumulate(
            chain([0], (width for width in cws))))
        self.recreate_all_selection_boxes()
        self.refresh_dropdowns()
    ...
    def reset_col_positions(self):
        ...
        if self.all_columns_displayed:
            self.col_positions = list(accumulate(
                chain([0], (colpos for c in range(self.total_data_cols())))))
        else:
            self.col_positions = list(accumulate(
                chain([0], (colpos for c in range(len(self.displayed_columns))))))
    ...
    def insert_col_positions(self, idx="end", widths=None, deselect_all=False):
        ...
        if idx == "end" or len(self.col_positions) == idx + 1:
            ...
            if len(w) > 1:
                idx += 1
                self.col_positions[idx:idx] = list(accumulate(
                    chain([self.col_positions[idx - 1] + w[0]], islice(w, 1, None))))
                idx += len(w)
                sumw = sum(w)
                self.col_positions[idx:] = [
                    e + sumw for e in islice(self.col_positions, idx, len(self.col_positions))]
            else:
                w = w[0]
                idx += 1
                self.col_positions.insert(idx, self.col_positions[idx - 1] + w)
    ...
    def insert_col_rc(self, event=None):
        if self.anything_selected(exclude_rows=True, exclude_cells=True):
            ...
            numcols = len(selcols)
            displayed_ins_col = min(
                selcols) if event == "left" else max(selcols) + 1
            if self.all_columns_displayed:
        ...
        if isinstance(self.paste_insert_column_limit, int) and self.paste_insert_column_limit < displayed_ins_col + numcols:
            numcols = self.paste_insert_column_limit - \
                len(self.col_positions) - 1
            if numcols < 1:
                return
        ...
        if not self.all_columns_displayed:
            try:
                ...
                disp_next = 0
            self.displayed_columns.extend(
                list(range(disp_next, disp_next + numcols)))
        ...
        self.col_options = {cn if cn < displayed_ins_col else cn +
                            numcols: t for cn, t in self.col_options.items()}
        self.CH.cell_options = {cn if cn < displayed_ins_col else cn +
                                numcols: t for cn, t in self.CH.cell_options.items()}
        self.refresh_dropdowns()
        if self.my_hdrs and isinstance(self.my_hdrs, list):
            try:
                self.my_hdrs[data_ins_col:data_ins_col] = list(
                    repeat("", numcols))
            except:
                pass
        if self.row_positions == [0] and not self.data_ref:
            ...
            for rn in range(len(self.data_ref)):
                self.data_ref[rn][data_ins_col:data_ins_col] = list(
                    repeat("", numcols))
        self.create_selected(0, displayed_ins_col, len(
            self.row_positions) - 1, displayed_ins_col + numcols, "cols")
    ...
    def insert_row_rc(self, event=None):
        ...
        if self.extra_begin_insert_rows_rc_func is not None:
            try:
                self.extra_begin_insert_rows_rc_func(
                    ("begin_insert_rows", stidx, posidx, numrows))
            except:
                return
        ...
        self.row_options = {rn if rn < posidx else rn +
                            numrows: t for rn, t in self.row_options.items()}
        self.RI.cell_options = {rn if rn < posidx else rn +
                                numrows: t for rn, t in self.RI.cell_options.items()}
        self.refresh_dropdowns()
        ...
        if self.col_positions == [0] and not self.data_ref:
            ...
            total_data_cols = self.total_data_cols()
            self.data_ref[stidx:stidx] = [
                list(repeat("", total_data_cols)) for rn in range(numrows)]
        self.create_selected(posidx, 0, posidx + numrows,
                             len(self.col_positions) - 1, "rows")
        ...
        if self.extra_end_insert_rows_rc_func is not None:
            self.extra_end_insert_rows_rc_func(
                ("end_insert_rows", stidx, posidx, numrows))
            ...
            if self.extra_begin_del_cols_rc_func is not None:
                try:
                    self.extra_begin_del_cols_rc_func(
                        ("begin_delete_columns", seld_cols))
                except:
                    return
            seldset = set(seld_cols) if self.all_columns_displayed else set(
                self.displayed_columns[c] for c in seld_cols)
            del_cell_options = tuple((r, c) for (
                r, c) in self.cell_options if c in seldset)
            ...
            if self.all_columns_displayed:
                if self.undo_enabled:
                    for c in reversed(seld_cols):
                        undo_storage['colwidths'][c] = self.col_positions[c +
                                                                          1] - self.col_positions[c]
                        for rn in range(len(self.data_ref)):
                            if c not in undo_storage['deleted_cols']:
                                undo_storage['deleted_cols'][c] = {}
                            try:
                                undo_storage['deleted_cols'][c][rn] = self.data_ref[rn].pop(
                                    c)
                            except:
                                continue
                    if self.my_hdrs and isinstance(self.my_hdrs, list):
                        for c in reversed(seld_cols):
                            try:
                                undo_storage['deleted_hdr_values'][c] = self.my_hdrs.pop(
                                    c)
                            except:
                                continue
                else:
                ...
                if self.undo_enabled:
                    for c in reversed(seld_cols):
                        ...
                        for rn in range(len(self.data_ref)):
                            ...
                            try:
                                undo_storage['deleted_cols'][self.displayed_columns[c]][rn] = self.data_ref[rn].pop(
                    ...
                    if self.my_hdrs and isinstance(self.my_hdrs, list):
                        for c in reversed(seld_cols):
                            try:
                                undo_storage['deleted_hdr_values'][self.displayed_columns[c]] = self.my_hdrs.pop(
                                    self.displayed_columns[c])
                            except:
                                continue
                else:
            ...
            if self.extra_begin_del_rows_rc_func is not None:
                try:
                    self.extra_begin_del_rows_rc_func(
                        ("begin_delete_rows", seld_rows))
                except:
                    return
            seldset = set(seld_rows)
            del_cell_options = tuple((r, c) for (
                r, c) in self.cell_options if r in seldset)
            for r, c in del_cell_options:
                if 'dropdown' in self.cell_options[(r, c)]:
                    self.destroy_dropdown(r, c)
            if self.undo_enabled:
                ...
                for r in reversed(seld_rows):
                    undo_storage['deleted_rows'].append((r, self.data_ref.pop(
                        r), self.row_positions[r + 1] - self.row_positions[r]))
            else:
                for r in reversed(seld_rows):
                    del self.data_ref[r]
            if self.my_row_index and isinstance(self.my_row_index, list):
    ...
    def reset_row_positions(self):
        ...
        self.row_positions = list(accumulate(
            chain([0], (rowpos for r in range(self.total_data_rows())))))
    ...
    def insert_row_positions(self, idx="end", heights=None, deselect_all=False):
        ...
        if idx == "end" or len(self.row_positions) == idx + 1:
            ...
            if len(h) > 1:
                idx += 1
                self.row_positions[idx:idx] = list(accumulate(
                    chain([self.row_positions[idx - 1] + h[0]], islice(h, 1, None))))
                idx += len(h)
                sumh = sum(h)
                self.row_positions[idx:] = [
                    e + sumh for e in islice(self.row_positions, idx, len(self.row_positions))]
            else:
                h = h[0]
                idx += 1
                self.row_positions.insert(idx, self.row_positions[idx - 1] + h)
    ...
    def move_row_position(self, idx1, idx2):
        if not len(self.row_positions) <= 2:
            if idx1 < idx2:
                height = self.row_positions[idx1 +
                                            1] - self.row_positions[idx1]
                self.row_positions.insert(
                    idx2 + 1, self.row_positions.pop(idx1 + 1))
                for i in range(idx1 + 1, idx2 + 1):
                    self.row_positions[i] -= height
                self.row_positions[idx2 +
                                   1] = self.row_positions[idx2] + height
            else:
                height = self.row_positions[idx1 +
                                            1] - self.row_positions[idx1]
                self.row_positions.insert(
    ...
    def move_col_position(self, idx1, idx2):
        if not len(self.col_positions) <= 2:
            if idx1 < idx2:
                ...
                self.col_positions.insert(
                    idx2 + 1, self.col_positions.pop(idx1 + 1))
                for i in range(idx1 + 1, idx2 + 1):
    ...
    def GetLinesHeight(self, n, old_method=False):
        if old_method:
            ...
            x = self.txt_measure_canvas.create_text(0, 0,
                                                    text="\n".join(
                                                        ["j^|" for lines in range(n)]) if n > 1 else "j^|",
                                                    font=self.my_font)
    ...
    def display_columns(self, indexes=None, enable=None, reset_col_positions=True, set_col_positions=True, deselect_all=True):
        ...
        if indexes is not None:
            ...
            if self.displayed_columns:
                self.hidd_col_expander_idxs = [c1 for c1, c2 in zip(islice(
                    self.displayed_columns, 0, None), islice(self.displayed_columns, 1, None)) if c2 - c1 != 1]
    ...
    def headers(self, newheaders=None, index=None, reset_col_positions=False, show_headers_if_not_sheet=True):
        if newheaders is not None:
            if isinstance(newheaders, (list, tuple)):
                self.my_hdrs = list(newheaders) if isinstance(
                    newheaders, tuple) else newheaders
            ...
            if reset_col_positions:
                ...
                if self.all_columns_displayed:
                    self.col_positions = list(accumulate(
                        chain([0], (colpos for c in range(len(self.my_hdrs))))))
                else:
                    self.col_positions = list(accumulate(
                        chain([0], (colpos for c in range(len(self.displayed_columns))))))
        else:
    ...
    def row_index(self, newindex=None, index=None, reset_row_positions=False, show_index_if_not_sheet=True):
        if newindex is not None:
            ...
            if isinstance(newindex, (list, tuple)):
                self.my_row_index = list(newindex) if isinstance(
                    newindex, tuple) else newindex
            ...
            if reset_row_positions:
                ...
                rowpos = self.default_rh[1]
                self.row_positions = list(accumulate(
                    chain([0], (rowpos for c in range(len(self.my_row_index))))))
        else:
            if index is not None:
                if isinstance(index, int):
        ...
        if total_rows is not None:
            if len(self.data_ref) < total_rows:
                if total_columns is None:
                    total_data_cols = self.total_data_cols()
                    self.data_ref.extend([list(repeat("", total_data_cols)) for r in range(
                        total_rows - len(self.data_ref))])
                else:
                    self.data_ref.extend([list(repeat("", total_columns))
                                         for r in range(total_rows - len(self.data_ref))])
            else:
                self.data_ref[total_rows:] = []
        if total_columns is not None:
            self.data_ref[:] = [r[:total_columns] if len(
                r) > total_columns else r + list(repeat("", total_columns - len(r))) for r in self.data_ref]

    def equalize_data_row_lengths(self):
        total_columns = self.total_data_cols()
        self.data_ref[:] = [r + list(repeat("", total_columns - len(r)))
                            if total_columns > len(r) else r for r in self.data_ref]
    ...
    def redraw_highlight_get_text_fg(self, r, c, fc, fr, sc, sr, c_2_, c_3_, c_4_, selected_cells, actual_selected_rows, actual_selected_cols, dcol):
        ...
        if (r, dcol) in self.cell_options and 'highlight' in self.cell_options[(r, dcol)] and (r, c) in selected_cells:
            if self.cell_options[(r, dcol)]['highlight'][0] is not None:
                c_1 = self.cell_options[(r, dcol)]['highlight'][0] if self.cell_options[(
                    r, dcol)]['highlight'][0].startswith("#") else Color_Map_[self.cell_options[(r, dcol)]['highlight'][0]]
            ...
            tf = self.table_selected_cells_fg if self.cell_options[(
                r, dcol)]['highlight'][1] is None or self.display_selected_fg_over_highlights else self.cell_options[(r, dcol)]['highlight'][1]
            ...
            if self.row_options[r]['highlight'][0] is not None:
                c_1 = self.row_options[r]['highlight'][0] if self.row_options[r]['highlight'][0].startswith(
                    "#") else Color_Map_[self.row_options[r]['highlight'][0]]
                self.redraw_highlight(fc + 1, fr + 1, sc, sr, fill=(f"#{int((int(c_1[1:3], 16) + c_2_[0]) / 2):02X}" +
                                                                    f"{int((int(c_1[3:5], 16) + c_2_[1]) / 2):02X}" +
                                                                    f"{int((int(c_1[5:], 16) + c_2_[2]) / 2):02X}"), outline="", tag="hi")
            tf = self.table_selected_cells_fg if self.row_options[r]['highlight'][
                1] is None or self.display_selected_fg_over_highlights else self.row_options[r]['highlight'][1]
            ...
            if self.col_options[dcol]['highlight'][0] is not None:
                c_1 = self.col_options[dcol]['highlight'][0] if self.col_options[dcol]['highlight'][0].startswith(
                    "#") else Color_Map_[self.col_options[dcol]['highlight'][0]]
                self.redraw_highlight(fc + 1, fr + 1, sc, sr, fill=(f"#{int((int(c_1[1:3], 16) + c_2_[0]) / 2):02X}" +
                                                                    f"{int((int(c_1[3:5], 16) + c_2_[1]) / 2):02X}" +
                                                                    f"{int((int(c_1[5:], 16) + c_2_[2]) / 2):02X}"), outline="", tag="hi")
            tf = self.table_selected_cells_fg if self.col_options[dcol]['highlight'][
                1] is None or self.display_selected_fg_over_highlights else self.col_options[dcol]['highlight'][1]
        ...
        # ________________________ CELL IS HIGHLIGHTED AND IN SELECTED ROWS ________________________
            ...
            tf = self.table_selected_rows_fg if self.cell_options[(
                r, dcol)]['highlight'][1] is None or self.display_selected_fg_over_highlights else self.cell_options[(r, dcol)]['highlight'][1]
            ...
            if self.row_options[r]['highlight'][0] is not None:
                ...
                self.redraw_highlight(fc + 1, fr + 1, sc, sr, fill=(f"#{int((int(c_1[1:3], 16) + c_4_[0]) / 2):02X}" +
                                                                    f"{int((int(c_1[3:5], 16) + c_4_[1]) / 2):02X}" +
                                                                    f"{int((int(c_1[5:], 16) + c_4_[2]) / 2):02X}"), outline="", tag="hi")
            tf = self.table_selected_rows_fg if self.row_options[r]['highlight'][
                1] is None or self.display_selected_fg_over_highlights else self.row_options[r]['highlight'][1]
            ...
            if self.col_options[dcol]['highlight'][0] is not None:
                ...
                self.redraw_highlight(fc + 1, fr + 1, sc, sr, fill=(f"#{int((int(c_1[1:3], 16) + c_4_[0]) / 2):02X}" +
                                                                    f"{int((int(c_1[3:5], 16) + c_4_[1]) / 2):02X}" +
                                                                    f"{int((int(c_1[5:], 16) + c_4_[2]) / 2):02X}"), outline="", tag="hi")
            tf = self.table_selected_rows_fg if self.col_options[dcol]['highlight'][
                1] is None or self.display_selected_fg_over_highlights else self.col_options[dcol]['highlight'][1]
        ...
        # ________________________ CELL IS HIGHLIGHTED AND IN SELECTED COLUMNS ________________________
            ...
            tf = self.table_selected_columns_fg if self.cell_options[(
                r, dcol)]['highlight'][1] is None or self.display_selected_fg_over_highlights else self.cell_options[(r, dcol)]['highlight'][1]
            ...
            if self.row_options[r]['highlight'][0] is not None:
                ...
                self.redraw_highlight(fc + 1, fr + 1, sc, sr, fill=(f"#{int((int(c_1[1:3], 16) + c_3_[0]) / 2):02X}" +
                                                                    f"{int((int(c_1[3:5], 16) + c_3_[1]) / 2):02X}" +
                                                                    f"{int((int(c_1[5:], 16) + c_3_[2]) / 2):02X}"), outline="", tag="hi")
            tf = self.table_selected_columns_fg if self.row_options[r]['highlight'][
                1] is None or self.display_selected_fg_over_highlights else self.row_options[r]['highlight'][1]
            ...
            if self.col_options[dcol]['highlight'][0] is not None:
                ...
                self.redraw_highlight(fc + 1, fr + 1, sc, sr, fill=(f"#{int((int(c_1[1:3], 16) + c_3_[0]) / 2):02X}" +
                                                                    f"{int((int(c_1[3:5], 16) + c_3_[1]) / 2):02X}" +
                                                                    f"{int((int(c_1[5:], 16) + c_3_[2]) / 2):02X}"), outline="", tag="hi")
            tf = self.table_selected_columns_fg if self.col_options[dcol]['highlight'][
                1] is None or self.display_selected_fg_over_highlights else self.col_options[dcol]['highlight'][1]
        ...
        # ________________________ CELL IS HIGHLIGHTED AND NOT SELECTED ________________________
            ...
            if self.cell_options[(r, dcol)]['highlight'][0] is not None:
                self.redraw_highlight(
                    fc + 1, fr + 1, sc, sr, fill=self.cell_options[(r, dcol)]['highlight'][0], outline="", tag="hi")
            tf = self.table_fg if self.cell_options[(
                r, dcol)]['highlight'][1] is None else self.cell_options[(r, dcol)]['highlight'][1]
            ...
            if self.row_options[r]['highlight'][0] is not None:
                self.redraw_highlight(
                    fc + 1, fr + 1, sc, sr, fill=self.row_options[r]['highlight'][0], outline="", tag="hi")
            tf = self.table_fg if self.row_options[r]['highlight'][
                1] is None else self.row_options[r]['highlight'][1]
            ...
            if self.col_options[dcol]['highlight'][0] is not None:
                self.redraw_highlight(
                    fc + 1, fr + 1, sc, sr, fill=self.col_options[dcol]['highlight'][0], outline="", tag="hi")
            tf = self.table_fg if self.col_options[dcol]['highlight'][
                1] is None else self.col_options[dcol]['highlight'][1]
    ...
    def redraw_highlight(self, x1, y1, x2, y2, fill, outline, tag):
        if self.hidd_high:
            ...
            t = self.create_rectangle(
                x1, y1, x2, y2, fill=fill, outline=outline, tag=tag)
    ...
    def main_table_redraw_grid_and_text(self, redraw_header=False, redraw_row_index=False, redraw_table=True):
        ...
        try:
            ...
            if can_width >= last_col_line_pos + self.empty_horizontal and self.parentframe.xscroll_showing:
                ...
                self.parentframe.xscroll.grid(
                    row=2, column=1, columnspan=2, sticky="nswe")
            ...
            for r in range(start_row - 1, end_row):
                ...
                if self.hidd_grid:
                    ...
                    if sh:
                        ...
                        self.itemconfig(
                            t, fill=self.table_grid_fg, state="normal")
                    self.disp_grid[t] = True
                else:
                    self.disp_grid[self.create_line(
                        x1, y, x_stop, y, fill=self.table_grid_fg, width=1, tag="g")] = True
        if self.show_vertical_grid:
            for c in range(start_col - 1, end_col):
                ...
                if self.hidd_grid:
                    ...
                    self.disp_grid[self.create_line(
                        x, y1, x, y_stop, fill=self.table_grid_fg, width=1, tag="g")] = True
        if start_row > 0:
            start_row -= 1
        if start_col > 0:
            start_col -= 1
        end_row -= 1
        c_2 = self.table_selected_cells_bg if self.table_selected_cells_bg.startswith(
            "#") else Color_Map_[self.table_selected_cells_bg]
        c_2_ = (int(c_2[1:3], 16), int(c_2[3:5], 16), int(c_2[5:], 16))
        c_3 = self.table_selected_columns_bg if self.table_selected_columns_bg.startswith(
            "#") else Color_Map_[self.table_selected_columns_bg]
        c_3_ = (int(c_3[1:3], 16), int(c_3[3:5], 16), int(c_3[5:], 16))
        c_4 = self.table_selected_rows_bg if self.table_selected_rows_bg.startswith(
        ...
        selected_cells, selected_rows, selected_cols, actual_selected_rows, actual_selected_cols = self.get_redraw_selections(
        ...
        if redraw_table:
            for c in range(start_col, end_col - 1):
                for r in rows_:
                    ...
                    tf = self.redraw_highlight_get_text_fg(
                        r, c, fc, fr, sc, sr, c_2_, c_3_, c_4_, selected_cells, actual_selected_rows, actual_selected_cols, dcol)
                    try:
                        if cell_alignment == "w":
                            if x > x2 or mw <= 5:
                                continue
                            lns = self.data_ref[r][dcol].split("\n") if isinstance(
                                self.data_ref[r][dcol], str) else f"{self.data_ref[r][dcol]}".split("\n")
                            y = fr + self.fl_ins
                            if y + self.half_txt_h - 1 > y1:
                                ...
                                if self.hidd_text:
                                    ...
                                    if sh:
                                        self.itemconfig(
                                            t, text=txt, fill=tf, font=self.my_font, anchor="w")
                                    else:
                                        self.itemconfig(
                                            t, text=txt, fill=tf, font=self.my_font, anchor="w", state="normal")
                                else:
                                    t = self.create_text(
                                        x, y, text=txt, fill=tf, font=self.my_font, anchor="w", tag="t")
                            ...
                            if len(lns) > 1:
                                stl = int(
                                    (y1 - y) / self.xtra_lines_increment) - 1
                                if stl < 1:
                                    stl = 1
                            ...
                            if y + self.half_txt_h - 1 > y1:
                                ...
                                if self.hidd_text:
                                    ...
                                    if sh:
                                        self.itemconfig(
                                            t, text=txt, fill=tf, font=self.my_font, anchor="e")
                                    else:
                                        self.itemconfig(
                                            t, text=txt, fill=tf, font=self.my_font, anchor="e", state="normal")
                                else:
                                    t = self.create_text(
                                        x, y, text=txt, fill=tf, font=self.my_font, anchor="e", tag="t")
                                self.disp_text[t] = True
                                wd = self.bbox(t)
                                wd = wd[2] - wd[0]
                                if wd > mw:
                            ...
                            if y + self.half_txt_h - 1 > y1:
                                if self.hidd_text:
                                    ...
                                    if sh:
                                        self.itemconfig(
                                            t, text=txt, fill=tf, font=self.my_font, anchor="center")
                                    else:
                                        self.itemconfig(
                                            t, text=txt, fill=tf, font=self.my_font, anchor="center", state="normal")
                                else:
                                    t = self.create_text(
                                        x, y, text=txt, fill=tf, font=self.my_font, anchor="center", tag="t")
                                self.disp_text[t] = True
                                wd = self.bbox(t)
                                wd = wd[2] - wd[0]
                                if wd > mw:
        ...
        try:
            ...
            if redraw_header and self.show_header:
                self.CH.redraw_grid_and_text(last_col_line_pos, x1, x_stop, start_col,
                                             end_col, selected_cols, actual_selected_rows, actual_selected_cols)
            if redraw_row_index and self.show_index:
                self.RI.redraw_grid_and_text(last_row_line_pos, y1, y_stop, start_row, end_row + 1,
                                             y2, x1, x_stop, selected_rows, actual_selected_cols, actual_selected_rows)
    ...
    def get_boxes(self):
        ...
        for item in self.get_all_selection_items():
            ...
            if alltags[0] == "CellSelectFill":
                ...
                boxes[tuple(int(e)
                            for e in alltags[1].split("_") if e)] = "rows"
            elif alltags[0] == "ColSelectFill":
                boxes[tuple(int(e)
                            for e in alltags[1].split("_") if e)] = "cols"
            elif alltags[0] == "Current_Inside":
                boxes[tuple(int(e) for e in alltags[1].split(
                    "_") if e)] = f"{alltags[2]}_inside"
            elif alltags[0] == "Current_Outside":
                boxes[tuple(int(e) for e in alltags[1].split(
                    "_") if e)] = f"{alltags[2]}_outside"
        return boxes
    ...
    def reselect_from_get_boxes(self, boxes):
        for k, v in boxes.items():
            if v == "cells":
                ...
                x = v.split("_")
                self.create_current(k[0], k[1], type_=x[0],
                                    inside=True if x[1] == "inside" else False)
    ...
    def delete_selection_rects(self, cells=True, rows=True, cols=True, delete_current=True):
        ...
        if cells:
            for item in self.find_withtag("CellSelectFill"):
                ...
                if alltags:
                    deleted_boxes[tuple(
                        int(e) for e in alltags[1].split("_") if e)] = "cells"
        ...
        if rows:
            for item in self.find_withtag("RowSelectFill"):
                ...
                if alltags:
                    deleted_boxes[tuple(
                        int(e) for e in alltags[1].split("_") if e)] = "rows"
        ...
        if cols:
            for item in self.find_withtag("ColSelectFill"):
                ...
                if alltags:
                    deleted_boxes[tuple(
                        int(e) for e in alltags[1].split("_") if e)] = "cols"
    ...
    def currently_selected(self, get_coords=False):
        items = self.find_withtag("Current_Inside") + \
            self.find_withtag("Current_Outside")
        if not items:
            return tuple()
        alltags = self.gettags(items[0])
    ...
    def set_current_to_last(self):
        if not self.currently_selected():
            items = sorted(self.find_withtag("CellSelectFill") + self.find_withtag(
                "RowSelectFill") + self.find_withtag("ColSelectFill"))
            if items:
    ...
    def create_selected(self, r1=None, c1=None, r2=None, c2=None, type_="cells", taglower=True):
        ...
        if currently_selected and isinstance(currently_selected[0], int):
            if (currently_selected[0] >= r1 and
                ...
                self.create_current(
                    currently_selected[0], currently_selected[1], type_="cell", inside=True)
        if type_ == "cells":
    ...
    def recreate_all_selection_boxes(self):
        for item in chain(self.find_withtag("CellSelectFill"),
            ...
            if full_tags:
                type_ = full_tags[0]
                r1, c1, r2, c2 = tuple(int(e)
                                       for e in full_tags[1].split("_") if e)
                self.delete(f"{r1}_{c1}_{r2}_{c2}")
                self.RI.delete(f"{r1}_{c1}_{r2}_{c2}")
                self.CH.delete(f"{r1}_{c1}_{r2}_{c2}")
    ...
    def get_redraw_selections(self, within_range):
        ...
        for item in self.find_withtag("RowSelectFill"):
            r1, c1, r2, c2 = tuple(int(e)
                                   for e in self.gettags(item)[1].split("_") if e)
            if (r1 >= within_r1 or
        ...
        for item in self.find_withtag("Current_Outside"):
            ...
            if (c1 >= within_c1 or
                ...
                scols.update(set(range(start_col, end_col)))
        if not self.show_selected_cells_border:
            iterable = chain(self.find_withtag("CellSelectFill"),
                             self.find_withtag("Current_Outside"))
        else:
        ...
        if get_cells:
            if within_range is None:
                for item in self.find_withtag("RowSelectFill"):
                    ...
                    s.update(
                        set(product(range(r1, r2), range(0, len(self.col_positions) - 1))))
                ...
                for item in self.find_withtag("RowSelectFill"):
                    ...
                    if (r1 >= within_r1 or
                        ...
                        if r2 < within_r2:
                            ...
                            end_row = within_r2
                        s.update(set(product(range(start_row, end_row),
                                 range(0, len(self.col_positions) - 1))))
                if get_cells_as_rows:
                    s.update(self.get_selected_cells(within_range=(
                        within_r1, 0, within_r2, len(self.col_positions) - 1)))
        else:
            if within_range is None:
                for item in self.find_withtag("RowSelectFill"):
                    r1, c1, r2, c2 = tuple(
            ...
            if within_range is None:
                for item in self.find_withtag("ColSelectFill"):
                    r1, c1, r2, c2 = tuple(
                        int(e) for e in self.gettags(item)[1].split("_") if e)
                    s.update(
                        set(product(range(c1, c2), range(0, len(self.row_positions) - 1))))
                ...
                for item in self.find_withtag("ColSelectFill"):
                    ...
                    if (c1 >= within_c1 or
                        ...
                        if c2 < within_c2:
                            ...
                            end_col = within_c2
                        s.update(set(product(range(start_col, end_col),
                                 range(0, len(self.row_positions) - 1))))
                if get_cells_as_cols:
                    s.update(self.get_selected_cells(within_range=(
                        0, within_c1, len(self.row_positions) - 1, within_c2)))
        else:
            if within_range is None:
                for item in self.find_withtag("ColSelectFill"):
                    r1, c1, r2, c2 = tuple(
    ...
    def get_selected_cells(self, get_rows=False, get_cols=False, within_range=None):
        ...
        if get_cols and get_rows:
            iterable = chain(self.find_withtag("CellSelectFill"), self.find_withtag(
                "RowSelectFill"), self.find_withtag("ColSelectFill"), self.find_withtag("Current_Outside"))
        elif get_rows and not get_cols:
            iterable = chain(self.find_withtag("CellSelectFill"), self.find_withtag(
                "RowSelectFill"), self.find_withtag("Current_Outside"))
        elif get_cols and not get_rows:
            iterable = chain(self.find_withtag("CellSelectFill"), self.find_withtag(
                "ColSelectFill"), self.find_withtag("Current_Outside"))
        else:
            iterable = chain(self.find_withtag("CellSelectFill"),
                             self.find_withtag("Current_Outside"))
        if within_range is None:
            ...
            for item in iterable:
                ...
                if (r1 >= within_r1 or
                    ...
                    if c2 < within_c2:
                        ...
                        end_col = within_c2
                    s.update(
                        set(product(range(start_row, end_row), range(start_col, end_col))))
        return s
        ...
        for item in sorted(self.find_withtag("CellSelectFill") + self.find_withtag("RowSelectFill") + self.find_withtag("ColSelectFill") + self.find_withtag("Current_Outside")):
            ...
            if tags:
                if tags[0].startswith(("Cell", "Current")):
                    boxes.append((tuple(int(e)
                                 for e in tags[1].split("_") if e), "cells"))
                elif tags[0].startswith("Row"):
                    boxes.append((tuple(int(e)
                                 for e in tags[1].split("_") if e), "rows"))
                elif tags[0].startswith("Col"):
                    boxes.append((tuple(int(e)
                                 for e in tags[1].split("_") if e), "cols"))
        return boxes
    ...
    def cell_selected(self, r, c, inc_cols=False, inc_rows=False):
        if not inc_cols and not inc_rows:
            ...
            iterable = chain(self.find_withtag("ColSelectFill"), self.find_withtag(
                "CellSelectFill"), self.find_withtag("Current_Inside"), self.find_withtag("Current_Outside"))
        elif not inc_cols and inc_rows:
            iterable = chain(self.find_withtag("RowSelectFill"), self.find_withtag(
                "CellSelectFill"), self.find_withtag("Current_Inside"), self.find_withtag("Current_Outside"))
        elif inc_cols and inc_rows:
            iterable = chain(self.find_withtag("RowSelectFill"), self.find_withtag("ColSelectFill"), self.find_withtag(
                "CellSelectFill"), self.find_withtag("Current_Inside"), self.find_withtag("Current_Outside"))
        for item in iterable:
            r1, c1, r2, c2 = tuple(int(e)
                                   for e in self.gettags(item)[1].split("_") if e)
            ...
            if f"{event.keysym}".lower() == "backspace":
                ...
                if self.cell_auto_resize_enabled:
                    self.set_cell_size_to_text(
                        y1, x1, only_set_if_too_small=True, redraw=True, run_binding=True)
        self.select_cell(r=y1, c=x1, keep_other_selections=True)
        self.create_text_editor(r=y1, c=x1, text=text,
    ...
    def create_text_editor(self, r=0, c=0, text=None, state="normal", see=True, set_data_ref_on_destroy=False, binding=None,
        ...
        if keep_existing_if_same_cell and (r, c) == self.text_editor_loc and self.text_editor is not None:
            self.text_editor.set_text(self.text_editor.get(
        ...
        self.text_editor = TextEditor(self, text=text, font=self.my_font, state=state, width=w, height=h,
                                      border_color=self.table_selected_cells_border_fg, show_border=self.show_selected_cells_border)
        ...
        self.text_editor.textedit.bind(
            "<Alt-Return>", self.text_editor_newline_binding)
        self.text_editor.textedit.bind(
            "<Option-Return>", self.text_editor_newline_binding)
        for key, func in self.text_editor_user_bound_keys.items():
            self.text_editor.textedit.bind(key, func)
        if binding is not None:
            self.text_editor.textedit.bind(
                "<Tab>", lambda x: binding((r, c, "Tab")))
            self.text_editor.textedit.bind(
            ...
            self.text_editor.textedit.bind(
                "<Tab>", lambda x: self.get_text_editor_value((r, c, "Tab")))
            self.text_editor.textedit.bind(
                "<Return>", lambda x: self.get_text_editor_value((r, c, "Return")))
            self.text_editor.textedit.bind(
                "<FocusOut>", lambda x: self.get_text_editor_value((r, c, "FocusOut")))
            self.text_editor.textedit.bind(
                "<Escape>", lambda x: self.get_text_editor_value((r, c, "Escape")))
        else:
            self.text_editor.textedit.bind(
    ...
    def destroy_text_editor(self, event=None):
        if event is not None and self.extra_end_edit_cell_func is not None:
            self.extra_end_edit_cell_func(
                self.text_editor_loc + ("Escape", None))
        self.text_editor_loc = None
        try:
    ...
    def text_editor_newline_binding(self, event=None):
        if self.GetLinesHeight(self.text_editor.get_num_lines() + 1) > self.text_editor.winfo_height():
            self.text_editor.config(
                height=self.text_editor.winfo_height() + self.xtra_lines_increment)
    ...
    def get_text_editor_value(self, destroy_tup=None, r=None, c=None, set_data_ref_on_destroy=True, event=None, destroy=True, move_down=True, redraw=True, recreate=True):
        ...
        if set_data_ref_on_destroy:
            ...
            if self.extra_end_edit_cell_func is not None:
                self.extra_end_edit_cell_func((r, c, destroy_tup[2] if len(
                    destroy_tup) >= 3 else "FocusOut", f"{self.text_editor_value}"))
        if move_down:
            if r is None and c is None and destroy_tup:
                r, c = destroy_tup[0], destroy_tup[1]
            currently_selected = self.currently_selected()
                ...
                if (
                    ...
                    if destroy_tup is not None and len(destroy_tup) >= 3 and destroy_tup[2] == "Return":
                        ...
                        self.see(r + 1 if r < len(self.row_positions) - 2 else r, c,
                                 keep_xscroll=True, bottom_right_corner=True, check_cell_visibility=True)
                    elif destroy_tup is not None and len(destroy_tup) >= 3 and destroy_tup[2] == "Tab":
                        self.select_cell(
                            r, c + 1 if c < len(self.col_positions) - 2 else c)
                        self.see(r, c + 1 if c < len(self.col_positions) - 2 else c,
                                 keep_xscroll=True, bottom_right_corner=True, check_cell_visibility=True)
        if recreate:
            self.recreate_all_selection_boxes()
            self.refresh_dropdowns()
    ...
    def set_cell_data(self, r=0, c=0, value="", undo=True, cell_resize=True):
        if r > len(self.data_ref) - 1:
            self.data_ref.extend([list(repeat("", c + 1))
                                 for i in range((r + 1) - len(self.data_ref))])
        elif c > len(self.data_ref[r]) - 1:
            self.data_ref[r].extend(
                list(repeat("", (c + 1) - len(self.data_ref[r]))))
    ...
    def create_dropdown(self, r=0, c=0, values=[], set_value=None, state="readonly", see=True, destroy_on_leave=True, destroy_on_select=True, current=False,
        ...
        quick_disp_cols = dict(
            zip(self.displayed_columns, range(len(self.displayed_columns))))
        if self.all_columns_displayed:
            cpos = c
        else:
        ...
        if values:
            self.set_cell_data(
                r, cpos, set_value if set_value is not None else values[0], cell_resize=False)
        if (r, c) not in self.cell_options:
            self.cell_options[(r, c)] = {}
        ...
        window = self.create_window((x, y),
                                    window=widget,
                                    anchor="nw")
        self.cell_options[(r, c)]['dropdown'] = (
            widget, window, selection_function)
    ...
    def get_dropdown_value(self, event=None, r=None, c=None, canvas_window=None, current=False, destroy=True, set_cell_on_select=True, redraw=True, recreate=True):
        ...
        if current:
            table_dropdown_value = self.cell_options[(
                r, c)]['dropdown'][0].dropdown.current()
        else:
            table_dropdown_value = self.cell_options[(
                r, c)]['dropdown'][0].get_my_value()
        # user has specified a selection function
        if self.cell_options[(r, c)]['dropdown'][2] is not None:
            self.cell_options[(r, c)]['dropdown'][2](
                (r, c, "ComboboxSelected", f"{table_dropdown_value}"))
        if set_cell_on_select:
            self.set_cell_data(r, cpos, table_dropdown_value,
                               cell_resize=True if destroy else False)
            if self.extra_end_edit_cell_func is not None:
                self.extra_end_edit_cell_func(
                    (r, c, "ComboboxSelected", f"{table_dropdown_value}"))
            self.focus_set()
            ...
            if 'dropdown' in self.cell_options[(r, c)]:
                if self.all_columns_displayed:
                    ...
                    if c not in quick_disp_cols:
                        try:
                            self.itemconfig(
                                self.cell_options[(r, c)]['dropdown'][1], state="hidden")
                            continue
                        except:
                            continue
                    cpos = quick_disp_cols[c]
                ...
                try:
                    ...
                    self.itemconfig(
                        self.cell_options[(r, c)]['dropdown'][1], width=w, height=h, state="normal")
                except:
                    continue
